...
export class UserMainContentComponent implements OnInit, OnChanges {
  ...
  AllCustomerNos: string[] = [];
  notificationSnackBarComponent: NotificationSnackBarComponent;
  fileToUpload: File;
  fileUploader: FileUploader;
  baseAddress: string;
  enableCustAndSalesField = false;
  slectedProfile: Uint8Array;
  authenticationDetails: AuthenticationDetails;
  MenuItems: string[];
  isCustomer: boolean;
  isSalesContractNumber: boolean;
  ...
  constructor(private _masterService: MasterService,
    ...
    this.isSalesContractNumber = false;
  }
  GetAllRoles(): void {
  ...
  ShowCustomerAndSalesContractFields(role: string): void {
    if (role.toLocaleUpperCase() === 'CUSTOMER') {
      this.enableCustAndSalesField = true;
    }
    else {
      this.enableCustAndSalesField = false;
    }
  }
  GetAllCustomerNos(): void {
    this._masterService.GetAllCustomerNos(this.authenticationDetails.userID).subscribe(
      (data) => {
        this.AllCustomerNos = <string[]>data;
        console.log(this.AllCustomerNos);
      },
      (err) => {
        console.log(err);
      }
  ...
  onRoleClick(selectedRoleName: string): void {
  ...
  loadSalesContractNo(customer: any): void {
    this._masterService.GetAllSalesContractNumber(this.authenticationDetails.userID, customer).subscribe(
      (data) => {
        const s = <string>data;
        this.userMainFormGroup.controls['SALES_CONTRACT_NO'].setValue(<string>data);
        console.log(s);
      },
      (err) => {
        console.log(err);
  ...
  ngOnInit(): void {
    ...
    this.GetAllRoles();
    this.GetAllCustomerNos();
    // this.userMainFormGroup.controls['contactNumber'].setValue('+225-');
  }
  ...
  SaveClicked(): void {
    if (this.userMainFormGroup.valid) {
      // console.log(this.userMainFormGroup.get('SALES_CONTRACT_NO').value);
      // if (this.isCustomer && !this.userMainFormGroup.get('SALES_CONTRACT_NO').value) {
      //   this.isSalesContractNumber = true;
      // } else {
        ...
        dialogRef.afterClosed().subscribe(
          result => {
            if (result) {
              ...
              this.user.CUSTOMER_NO = this.userMainFormGroup.get('CUSTOMER_NO').value;
              this.user.SALES_CONTRACT_NO = this.userMainFormGroup.get('SALES_CONTRACT_NO').value;
              // CUSTOMER_NO: ['', Validators.required],
              // SALES_CONTRACT_NO: ['', Validators.required],
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.user) {
      ...
      this.userMainFormGroup.get('CUSTOMER_NO').patchValue(this.user.CUSTOMER_NO);
      this.userMainFormGroup.get('SALES_CONTRACT_NO').patchValue(this.user.SALES_CONTRACT_NO);
      // const res = this.AllRoles.filter(x => x.RoleID === this.user.RoleID)[0];
      // if (res) {
      //   if (res.RoleName === 'Customer') {
      //     this.isCustomer = true;
      //   } else {
      //     this.isCustomer = false;
      //   }
      // }
      if (this.user.SALES_CONTRACT_NO && this.user.SALES_CONTRACT_NO !== 'null') {
        this.isCustomer = true;
      } else {
        this.isCustomer = false;
      }
